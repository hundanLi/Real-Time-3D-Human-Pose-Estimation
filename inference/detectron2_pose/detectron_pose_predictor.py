...
def get_img_paths(imgs_dir):
    ...
    for dirpath, dirnames, filenames in os.walk(imgs_dir):
        for filename in [f for f in filenames if
                         f.endswith('.png') or f.endswith('.PNG') or f.endswith('.jpg') or f.endswith(
                             '.JPG') or f.endswith('.jpeg') or f.endswith('.JPEG')]:
...
if __name__ == '__main__':
    ...
    img_generator = read_video('./input_video.mp4')  # or get them from a video
    output_path = './npz/pose2d'
