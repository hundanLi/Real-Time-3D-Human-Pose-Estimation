...
def render_image(coords_3d, skeleton, azim, input_video_frame, save=False, save_path=None):
    # 人数
...
posenet_name = ['simple_pose_resnet18_v1b', 'simple_pose_resnet101_v1b', 'simple_pose_resnet50_v1d_int8',
                'simple_pose_resnet101_v1d_int8', 'simple_pose_resnet18_v1b_int8']
detector = model_zoo.get_model(detector_name[0], pretrained=True)
pose_net = model_zoo.get_model(posenet_name[0], pretrained=True)
...
def predict_imgs(image_dir, result_dir):
    ...
    for parent, _, files in os.walk(image_dir):
        ...
        print("detector fps: {:.3f}; 2d predictor fps: {:.3f}; 3d predictor fps: {:.3f}; fps: {:.3f}".format(
            frames / detector_time,
