...
# frame = cv2.cvtColor(cv2.imread(im_fname), cv2.COLOR_BGR2RGB)
# x, img = data.transforms.presets.yolo.transform_test(nd.array(frame), short=360)
...
print("total elapsed time of 2d pose: {:.2f}".format(time.time() - start))
...
# 画图
# noinspection PyUnresolvedReferences
def render_image(keypoints, coords_3d, skeleton, azim, input_video_frame, save=True):
    ...
    pos = coords_3d['Reconstruction'][-1]
...
# 渲染图像
render_image(input_keypoints, coords_3d=pos_3d, skeleton=Skeleton(),
