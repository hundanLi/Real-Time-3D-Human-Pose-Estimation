...
def video_pose(filepath, ckpt_dir, ckpt_name, filter_widths, show=False):
    # 加载3d姿势估计器
...
def joints_2d_detector_test():
    im_filename = 'images/liuyifei1.jpg'
    frame = cv2.cvtColor(cv2.imread(im_filename), cv2.COLOR_BGR2RGB)
    # frame = mxnet.image.imread(im_filename)
    joints_dict = detect_2d_joints(frame)
...
if __name__ == '__main__':
    ...
    video_pose(video_path, ckpt_dir='../../checkpoint/detectron_pt_coco',
               ckpt_name='arc_27_epoch_40.bin', filter_widths=[3, 3, 3], show=False)
